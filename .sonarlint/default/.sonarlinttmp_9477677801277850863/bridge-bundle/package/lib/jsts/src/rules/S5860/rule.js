        ...
        if (regex) {
            ...
            if (indexedGroups.length > 0) {
                ...
                intellisense.context.report({
                    message: (0, helpers_1.toEncodedMessage)(`Directly use the group names instead of their numbers.`, locations, messages),
                    node: substr,
                });
            }
        }
    ...
    if (regex) {
        ...
        if (maybeIndex && typeof maybeIndex.value === 'number') {
            ...
            if (group) {
                ...
                intellisense.context.report({
                    message: (0, helpers_1.toEncodedMessage)(`Directly use '${group.name}' instead of its group number.`, locations, messages),
                    node: property,
                });
            }
        }
    ...
    if (regex) {
        ...
        for (const groupNode of groupNodes) {
            ...
            if (group) {
                ...
                intellisense.context.report({
                    message: (0, helpers_1.toEncodedMessage)(`There is no group named '${groupName}' in the regular expression.`, locations, messages),
                    node: groupNode,
                });
            }
        }
    ...
    intellisense.getKnowledge().forEach(regex => {
        if (regex.matched) {
            ...
            if (unusedGroups.length) {
                ...
                intellisense.context.report({
                    message: (0, helpers_1.toEncodedMessage)('Use the named groups of this regex or remove the names.', locations, messages),
                    node: regex.node,
                });
            }
        }
